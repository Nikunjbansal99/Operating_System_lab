...
int main()
{
    ...
    scanf("%d",&num_of_blk);
    int blockSize[num_of_blk];
    for(count=1;count<=num_of_blk;count++)
    {
        printf("Enter size of block %d is :",count);
        scanf("%d",&blockSize[count]);
    }
	...
	bestFit(blockSize, num_of_blk, processSize, pr);
